...
<link href="https://fonts.googleapis.com/css?family=Poppins:300,400"
	rel="stylesheet" />
...
<script>
						...
						function(index, item) {
							...
							if (index >= startPageGroup && index < endPageGroup) {
								context += "<tr><td class='srial'>"
										+ item.mainBoardNum + "</td>";
								context += "<td class='Session'><a href=listDetailGO?mainBoardNum="
										+ item.mainBoardNum
										+ ">"
										+ item.title
										+ "</a></td>";
								context += "<td class='Session'>"
										+ item.country + "</td>";
								context += "<td class='Session'>" + start + "~"
										+ end + "</td>";
								context += "<td class='Session'>" + item.adress
										+ "</td></tr>";
							}
						});
		$("#list").html(context);
		...
		if (searchItem == "hashSearch") {
			$('#hash')
					.append(
							"<span class='span'>"
									+ searchKeyword
									+ "<button id='xbtn' value="+searchKeyword+">X</button></span>");
	...
	function selectOne() {
		...
		if (searchItem == "hashSearch") {//해시태그 생성
			...
			idx += 1;
			$('#hash').append(
					"<span id="+sharp+">" + searchKeyword + "<button id=" + idx
							+ " value=" + searchKeyword + " onclick='btnClick("
							+ sharp + ")'>X</button></span>");
			selectHashtag(searchKeyword);
			return false;
		}
...
<body>
	...
	<div class="boxed_wrapper">
		...
		<section class="page-title"
			style="background: url(resources/images/userimage/등축제.jpg);">
